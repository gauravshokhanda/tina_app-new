...
const userSlice = createSlice({
    ...
    reducers: {
        ...
        updateProfileState: (state, action: PayloadAction<{ name: string; profileImage: string | null }>) => {
...
export const { setUserState, updateProfileState, clearUserState, loadUserState } = userSlice.actions;

// Async Thunks with correct types
export const setUser = (userData: UserState) => async (dispatch: AppDispatch) => {
    await AsyncStorage.setItem("user", JSON.stringify(userData));
    dispatch(setUserState(userData));
};

export const updateProfile = (updatedData: { name: string; profileImage: string | null }) => 
